...
func (peer *Peer) SendBuffer(buffer []byte) error {
	...
	encpacket := peer.device.auth.EncPacket(buffer)

	err := peer.device.net.bind.Send(encpacket, peer.endpoint)
	if err == nil {
		atomic.AddUint64(&peer.stats.txBytes, uint64(len(encpacket)))
	}
	return err
}
